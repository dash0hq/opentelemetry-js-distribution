...
export default class PodUidDetector implements ResourceDetector {
  detect(): DetectedResource {
    const podUidPromise = this.detectPodUid();
    const attrNames = [SEMRESATTRS_K8S_POD_UID];
    const attributes = {} as DetectedResourceAttributes;
    attrNames.forEach(name => {
      attributes[name] = podUidPromise.then(data => data[name]);
    });
    return { attributes };
  }

  private async detectPodUid(): Promise<DetectedResourceAttributes> {
